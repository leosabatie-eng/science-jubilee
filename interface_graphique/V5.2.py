...
def export_layout(filename="test1.json"):
    ...
    for slot_id, tool in slot_assignments.items():
        if tool != "None":
            data["tool_slots"][str(slot_id)] = tool



   # data["tool_slots"][str(slot_id)] = TOOL_SCIENTIFIC_NAMES
...
def load_json():
    ...
    for slot_id, tool in tool_slots.items():
        slot_id = int(slot_id)
        slot_assignments[slot_id] = "None" if tool is None else tool
        update_slot_display(slot_id)
...
def export_to_dxf(json_file="test1.json", out="plan.dxf"):
        try:
        ...
        def trous_de_fixation(x1, y1, x2, y2):
            ...
            positions = [
                (x1 + OFFSET_TROU_BAS_X , y1 - OFFSET_TROU_BAS_y),  # bas gauche
                (x2 - OFFSET_TROU_BAS_X, y1 - OFFSET_TROU_BAS_y),  # bas droit
                (x2 - OFFSET_TROU_HAUT_X, y2 + OFFSET_TROU_HAUT_Y),  # haut droit
                (x1 + OFFSET_TROU_HAUT_X, y2 + OFFSET_TROU_HAUT_Y),  # haut gauche
            ]
