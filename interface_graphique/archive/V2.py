...
class DraggableObject:
    ...
    def do_drag(event):
        global current_drag_id, drag_last_x, drag_last_y
        if current_drag_id is None:
            return

        dx = event.x - drag_last_x
        dy = event.y - drag_last_y

        meta = objects_meta.get(current_drag_id)
        if not meta:
            return

        # Calcul des nouvelles coordonnées après déplacement
        x1, y1, x2, y2 = canvas.coords(current_drag_id)
        new_x = x1 + dx
        new_y = y1 + dy
        w = x2 - x1
        h = y2 - y1

        if not is_inside_plateau(new_x, new_y, w, h):
            return  # ne bouge pas si ça sort du plateau

        if not is_free_space(new_x, new_y, w, h, ignore_id=current_drag_id):
            return  # ne bouge pas si ça chevauche

        # Déplacement autorisé
        canvas.move(meta["tag"], dx, dy)
        drag_last_x = event.x
        drag_last_y = event.y
...
def export_to_dxf(json_file="workspace_export.json", out="plan.dxf"):
    ...
    def draw_rectangle(x1, y1, x2, y2, angle_deg=0, layer="default"):
        ...
        cx = x1 + w/2
        cy = y1 + h/2
        pts = [(x1 - cx, y1 - cy), (x2 - cx, y1 - cy), (x2 - cx, y2 - cy), (x1 - cx, y2 - cy)]
